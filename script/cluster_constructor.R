...
#物种颜色设置
phylum_color <- c('#8DD3C7', '#FFFFB3', '#BEBADA', '#FB8072', '#80B1D3', '#FDB462', '#B3DE69', '#FCCDE5', '#BC80BD', '#CCEBC5', 'gray','black')
...
text(x = -0.05, y = bar, labels = colnames(otu), col =color , xpd = TRUE,font = 7)
#柱形图图例
par(mar = c(5, 1, 5, 0))
...
mtext('Top 10 phylums', side = 3, line = 1, cex = 1,font = 7)
text(x = -0.05, y = bar, labels = colnames(dat), col = group_col[group[tree$order, 2]], xpd = TRUE,font = 7)
#柱形图图例
par(mar = c(5, 1, 5, 0))
plot(0, type = 'n', xaxt = 'n', yaxt = 'n', bty = 'n', xlab = '', ylab = '')
legend('left', pch = 15, col = phylum_color, legend = names(phylum_color), bty = 'n', cex = 1,text.font = 7)
